...
} //using webpack es6 on front end.
...
var typeAhead = function typeAhead(search) {
  ...
  searchInput.on('input', function () {
    ...
    // note: using bling.js, instead of addEventListener
    if (!this.value) {
    ...
    _axios2.default.get('/api/search?q=' + this.value).then(function (res) {
      ...
      if (res.data.length) {
        searchResults.innerHTML = _dompurify2.default.sanitize(doHTML(res.data));
      } else {
  ...
  searchInput.on('keyup', function (e) {
    ...
    var next = void 0; //next (active) node in search result items
    var ac = 'search__result--active'; //activeClass
    var current = search.querySelector('.' + ac);
